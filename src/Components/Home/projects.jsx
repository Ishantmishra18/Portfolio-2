...
const Projects = () => {
  ...
  // Handle screen video play on hover
const handleScreenMouseEnter = (index) => {
  setHoveredScreen(index);
  
  // Play desktop video from current time if available
  if (videoRefs.current[index]) {
    videoRefs.current[index].play().catch(e => console.log("Video play failed:", e));
  }
};

// Handle screen video pause when not hovering
const handleScreenMouseLeave = (index) => {
  setHoveredScreen(null);
  
  // Just pause desktop video (don't reset time)
  if (videoRefs.current[index]) {
    videoRefs.current[index].pause();
    // REMOVE this line: videoRefs.current[index].currentTime = 0;
  }
};

// Handle mobile video play on hover
const handleMobileMouseEnter = (index) => {
  setHoveredMobile(index);
  
  // Play mobile video from current time if available
  if (mobileVideoRefs.current[index]) {
    mobileVideoRefs.current[index].play().catch(e => console.log("Mobile video play failed:", e));
  }
};

// Handle mobile video pause when not hovering
const handleMobileMouseLeave = (index) => {
  setHoveredMobile(null);
  
  // Just pause mobile video (don't reset time)
  if (mobileVideoRefs.current[index]) {
    mobileVideoRefs.current[index].pause();
    // REMOVE this line: mobileVideoRefs.current[index].currentTime = 0;
  }
};
                ...
                >
                    ...
                    <div className="procont flex flex-col items-center bg-neutral-800 backdrop-blur-lg pt-10 rounded-xl w-[96vw] md:w-[70vw]">
                        <div className="imgcont w-full md:h-[70vh] h-[30vh] relative bg-black">
                            {/* Mobile View with Video - Separate Hover */}
                            <div 
                                className={`mobileview absolute w-[18vw] px-1 pt-1 pb-3 h-[74vh] md:block hidden bg-zinc-800 -top-[10vh] rounded-2xl z-20 ${key%2==0?'-left-[3%]':'-right-[3%]'}`}
                                onMouseEnter={() => handleMobileMouseEnter(key)}
                                onMouseLeave={() => handleMobileMouseLeave(key)}
                            >
                                <div className="relative w-full h-full rounded-2xl shadow-xl overflow-hidden">
                                    {/* Fallback image - Always visible unless hovering */}
                                    <img 
                                        src={`/pro/${key+1}/mobile.png`} 
                                        alt="" 
                                        className={`object-cover h-full w-full rounded-2xl ${hoveredMobile === key ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}
                                    />
                                    
                                    {/* Video element - Only visible when hovering */}
                                    <video
                                        ref={el => mobileVideoRefs.current[key] = el}
                                        className={`absolute top-0 left-0 w-full h-full object-cover rounded-2xl ${hoveredMobile === key ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
                                        muted
                                        loop
                                        playsInline
                                        preload="auto"
                                    >
                                        <source src={`/pro/${key+1}/vidMobile.mp4`} type="video/mp4" />
                                        <source src={`/pro/${key+1}/vidMobile.webm`} type="video/webm" />
                            ...
                            {/* Desktop View with Video - Separate Hover */}
                            <div 
                                className="relative w-full h-full overflow-hidden"
                                onMouseEnter={() => handleScreenMouseEnter(key)}
                                onMouseLeave={() => handleScreenMouseLeave(key)}
                            >
                                {/* Fallback image - Always visible unless hovering */}
                                <img 
                                    src={`/pro/${key + 1}/screen.png`} 
                                    alt="" 
                                    className={`w-full h-full md:object-cover object-contain z-10 ${hoveredScreen === key ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}
                                />
                                
                                {/* Video element - Only visible when hovering */}
                                <video
                                    ref={el => videoRefs.current[key] = el}
                                    className={`absolute top-0 left-0 w-full h-full object-cover ${hoveredScreen === key ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
                                    muted
                                    loop
                                    playsInline
                                    preload="auto"
                                >
                                    <source src={`/pro/${key+1}/vidScreen.mp4`} type="video/mp4" />
                                    <source src={`/pro/${key+1}/vidScreen.webm`} type="video/webm" />
                                    Your browser does not support the video tag.
                                </video>
                            </div>
                        </div>
