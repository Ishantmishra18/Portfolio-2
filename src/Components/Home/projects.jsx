...
const Projects = () => {
    ...
    return (
        <div className='h-auto pb-32 bg-neutral-900  proPage w-screen rounded-t-[60px] text-white pt-[10vh]'>
            ...
            {projects.map((val, key) => (
                <div
                    key={key}
                    className={`pro h-auto py-10 w-screen relative px-[4%] flex flex-col bg-cover  ${key === 0 ? 'rounded-t-[60px]' : ''} ${key%2===0?'items-start':'items-end'}`}
                >
                  <div className="overlay absolute inset-0 bg-black blur-2xl opacity-80 rounded-t-[60px]"></div>
                    <div className={`proname pointer-events-none sticky top-[50%]  text-[6vw] z-30 flex flex-col  ${key%2===0?'left-[80vw] items-end':'right-[80vw] items-start'}`}>
                        <h2>{val}</h2>
                        <h2 className="date text-[1.2vw] text-neutral-600 mt-[-3vh]">2020 Aug</h2>
                        </div>
                    <div className="procont flex flex-col items-center bg-neutral-700 bg-opacity-25 backdrop-blur-lg pt-10 rounded-xl w-[70vw]">
                        ...
                        <div className="imgcont w-[100%] overflow-hidden relative" onMouseEnter={handleView} onMouseLeave={handleView}>
                            ...
                            <img src={`/pro/${key + 1}.png`} alt="" className='w-full rounded-x hover:scale-[1.02] z-10 duration-200' />
                        </div>
